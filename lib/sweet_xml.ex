...
defmodule SwXml do
  @moduledoc ~S"""
  `SwXml` is a thin wrapper around `:xmerl`. It allows you to convert a
  string or xmlElement record as defined in `:xmerl` to an elixir value such
  as `map`, `list`, `char_list`, or any combination of these.

  For normal sized documents, `SwXml` primarily exposes 3 functions

    * `SwXml.xpath/2` - return a value based on the xpath expression
    * `SwXml.xpath/3` - similar to above but allowing nesting of mapping
    * `SwXml.xmap/2` - return a map with keywords mapped to values returned
      from xpath

  For something larger, `SwXml` mainly exposes 1 function

    * `SwXml.stream_tags/3` - stream a given tag or a list of tags, and
  ...
  Streaming
      ...
      iex> SwXml.stream_tags(doc, :li)
      ...> |> Stream.map(fn {:li, doc} ->
      ...>      doc |> SwXml.xpath(~x"./text()")
      ...>    end)
      ...> |> Enum.to_list
      ['l1', 'l2', 'l3']
  ...
  Notice also in the examples section, we always import SwXml first. This
  makes `x_sigil` available in the current scope. Without it, instead of using
  `~x`, you can do the following

      iex> doc = "<h1><a>Some linked title</a></h1>"
      iex> doc |> SwXml.xpath(%SwXpath{path: '//a/text()', is_value: true, cast_to: false, is_list: false, is_keyword: false})
      'Some linked title'
  ...
  @doc ~s"""
  `sigil_x/2` simply returns a `SwXpath` struct, with modifiers converted to
  boolean fields

      iex> SwXml.sigil_x("//some/path", 'e')
      %SwXpath{path: '//some/path', is_value: false, cast_to: false, is_list: false, is_keyword: false}
  ...
  def add_namespace(xpath, prefix, uri) do
    %SwXpath{xpath | namespaces: [{to_charlist(prefix), to_charlist(uri)}
                                     | xpath.namespaces]}
  end
...
@doc """
  ...
  Examples:
      ...
      iex> import SwXml
      iex> doc = ["<ul><li>l1</li><li>l2", "</li><li>l3</li></ul>"]
      iex> SwXml.stream_tags(doc, :li, discard: [:li])
      ...> |> Stream.map(fn {:li, doc} -> doc |> SwXml.xpath(~x"./text()") end)
      ...> |> Enum.to_list
      ['l1', 'l2', 'l3']
      iex> SwXml.stream_tags(doc, [:ul, :li])
      ...> |> Stream.map(fn {_, doc} -> doc |> SwXml.xpath(~x"./text()") end)
      ...> |> Enum.to_list
      ['l1', 'l2', 'l3', nil]
  ...
  Examples:
      ...
      iex> SwXml.stream_tags(doc, :header)
      ...> |> Stream.map(fn {_, doc} -> SwXml.xpath(doc, ~x".//title/text()") end)
      ...> |> Enum.to_list
      ['Nested', 'XML']
      iex> SwXml.stream_tags(doc, :header, discard: [:title])
      ...> |> Stream.map(fn {_, doc} -> SwXml.xpath(doc, ~x"./title/text()") end)
      ...> |> Enum.to_list
      [nil, nil]
  ...
  This is a lower level API compared to `SwXml.stream_tags`. You can use
  ...
  The second argument to xpath is a `SwXpath` struct. The optional third
  argument is a keyword list, such that the value of each keyword is also
  either a `SwXpath` or a list with head being a `SwXpath` and tail being
  ...
  @doc """
  Tags `%SwXpath{}` with `fun` to be applied at the end of `xpath` query.
